...
class FirestoreDatabase {
  ...
  Future<Note> getNote(noteId) async {
    DocumentSnapshot doc =
        await noteCollection.doc(uid).collection("my_tasks").doc(noteId).get();
...
//
  ...
  Future<int> updateNote(noteId, Note note) async {
   ...
   //   UploadTask? task =
    ...
    await noteCollection.doc(uid).collection("my_tasks").doc(noteId).update({
      "title": note.title,
      "description": note.description,
      //"path": note.path
    });
  ...
  Future<int> deleteNote(noteId) async {
    await noteCollection.doc(uid).collection("my_tasks").doc(noteId).delete();
  ...
  Future<NoteCollection> getNoteList() async {
    QuerySnapshot snapshot =
        await noteCollection.doc(uid).collection("my_tasks").get();
